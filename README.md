...
- 材料规格拆分与合并：同单位批次直接改挂、跨单位逐批换算并保留不可变谱系事件。
- 批次入库、来源追溯、有效期和库存流水。
- 库存调整、自动耗尽、归档和并发余额保护。
- 项目、材料需求、计划与实际消耗对比。
- 消耗、损耗、撤销和反向库存流水。
...
- 规格拆分/合并是谱系事件（`material_events`），只追加、不更新、不删除；逐批落库单位换算前后的数量与单位。
- 同单位迁移保留批次与历史流水；跨单位迁移旧批次以 `TRANSFER_OUT` 清零归档、目标材料下以 `TRANSFER_IN` 建立承接批次。
- 拆分/合并只能在同一单位族内进行；合并时来源材料的项目需求同步改挂并换算计划数量。
- 归档代替核心数据硬删除。
- 材料归档后禁止新增批次或项目需求引用，由应用层与数据库约束触发器共同拦截；归档接口按 `version` 拒绝并发覆盖。
- 审计日志只追加，不更新、不删除。
