...
- 材料规格拆分与合并：只追加谱系、批次级单位映射、乐观版本并发保护。
- 批次入库、来源追溯、有效期和库存流水。
- 库存调整、自动耗尽、归档和并发余额保护。
- 项目、材料需求、计划与实际消耗对比。
- 消耗、损耗、撤销和反向库存流水。
...
- 归档代替核心数据硬删除。
- 规格拆分/合并不改动历史归属：来源批次记 `SPEC_TRANSFER_OUT` 后保留在原档案，目标材料另建新批次并以 `OPENING` 引用同一规格事件；逐批次的来源/目标数量与单位映射保存在谱系表中。
- 规格事件和材料归档都按材料 `version` 拒绝并发覆盖；来源材料全部库存转出后自动归档。
- 已归档材料不能再新增批次、项目需求、消耗、颜色变化或规格事件。
- 审计日志只追加，不更新、不删除。
